...
def convert_lambert2_to_long_lat(df_preci):
    ...
    # Configurer le transformateur pour passer de EPSG:27572 (Lambert II étendu) à EPSG:4326 (WGS 84)
    # Appliquer la transformation, x100 pour convertir les coordonnées Lambert (hm) en mètres
    transformer = Transformer.from_crs("EPSG:27572", "EPSG:4326", always_xy=True)
    df_preci = df_preci.copy()
    df_preci['LON'], df_preci['LAT'] = transformer.transform(df_preci['LAMBX']*100, df_preci['LAMBY']*100)
    columns= ['LAMBX', 'LAMBY', 'LON', 'LAT', 'DATE', 'PRENEI_Q', 'PRELIQ_Q', 'PE_Q']
...
def process_and_export_meteo_data(csv_compress, geojson_path, 
                                  start_date, 
                                  end_date,
    ...
    Paramètres :
      ...
      - centroid_geojson_json: Sauvegarde la liste des centres (coordonnées uniques) en JSON.
    """
